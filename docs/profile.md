...
The account name/avatar opens `/profile`. The three tabs contain:

- **Overview:** up to four favorite movies, watched/watchlist counts, total runtime of watched movies, average rating, top genres, daily-game statistics, account higher/lower record, recent activity, badges and a friend-comparison link.
...
| Endpoint | Behavior |
| --- | --- |
| `GET /api/profile` | Profile, aggregate summary, 12 latest activity entries and badge progress. |
...
Public profiles at `/u/[slug]` are opt-in. `GET /api/profiles/[slug]` exposes only nickname, bio, avatar, four favorites, watched count/total runtime, game statistics and unlocked badges. It excludes Clerk user ID, email, preferences, reviews, individual ratings and the full collection. Private/nonexistent profiles return 404; turning visibility off removes public access. The response is not cached, and visibility is rechecked after aggregate queries.

`GET /api/profiles/[slug]/compare` requires login and a different public profile. It returns common favorites/highly rated shared films, an aggregate similarity score (only with at least three shared ratings), and up to three joint suggestions. Exact ratings and the other person's private records are not returned. Suggestions exclude either person's watched/negatively rated films and feedback, respect both genre exclusions, the shorter runtime limit, and each person's selected providers. The similarity percentage measures agreement, not predictive accuracy.
