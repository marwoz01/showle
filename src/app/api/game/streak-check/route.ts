  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      const missedDays = Math.round(
        (Date.parse(yesterday) - Date.parse(lastPlayed)) / 86400000,
      );
      if (wallet && missedDays > 0 && wallet.streakFreezes >= missedDays) {
        ...
        await tx.coinTransaction.create({
          data: { userId, amount: 0, reason: "use_freeze", dateKey: today },
        });
        ...
        return {
          status: "freeze_used",
          remainingFreezes: wallet.streakFreezes - missedDays,
        };
      }
      await tx.userStats.update({
        where: { userId },
