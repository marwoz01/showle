...
const pl: Translations = {
  ...
  result: {
    ...
    dayStreak: (count) => `${count} ${count === 1 ? "dzień" : "dni"} z rzędu`,
    streakSaved: "Dzisiejsza gra zapisuje Twoją serię.",
    bestStreak: (count) => `Najlepsza seria: ${count} dni`,
    yourRanking: "Twój wynik",
    betterThan: (percent) => `Lepiej niż ${percent}% dzisiejszych graczy`,
    calculatingResult: "Porównujemy Twój wynik z innymi graczami…",
    playersToday: (count) => `${count} ${count === 1 ? "gracz dzisiaj" : "graczy dzisiaj"}`,
    answerDistribution: "Rozkład odpowiedzi",
    missedShort: "X",
    tomorrowTitle: "Jutro w Showle",
    tomorrowHint: (decade, genre) => `Film z lat ${decade}–${decade + 9} · ${genre}`,
    tomorrowMystery: "Nowa filmowa zagadka czeka za rogiem",
    comeBackTomorrow: "Wróć po północy, żeby podtrzymać serię.",
