...
Daily actor portraits compare each guessed person against the full answer credits, regardless of billing or role. TMDB details retain all credit names in `castNames` while displayed portraits remain limited to eight. `castComparison` returns only guessed names and their exact/miss status; the nine existing comparison fields and lead-actor reveal remain unchanged. Old snapshots enrich only the answer's credits through one cached, deduplicated lookup. If full credits are unavailable, known matches stay green and uncertain absences stay neutral. No extra lookup runs per portrait, and unguessed answer names are not added to the playing response.

Genre chips likewise receive individual exact/miss statuses for localized guessed genres only; the aggregate genre clue stays partial until both genre sets match, so missing answer genres remain hidden.
