    ...
    <div ref={topRef} className="relative mx-auto max-w-6xl space-y-7">
      <header>
        <div className="mb-3 flex items-center gap-3">
          <Link href="/recommend" aria-label={movieChoiceCopy[locale].back} className="rounded-lg p-2 text-muted hover:text-foreground"><ArrowLeft size={20} /></Link>
      ...
      {view === "error" && <div role="alert" className="soft-card space-y-5 rounded-2xl p-8 text-center">
        ...
        {meta && (Boolean(preferences.freeformText.trim()) || Boolean(preferences.referenceMovieId)) &&
          (meta.matching === "filters" || (Boolean(preferences.freeformText.trim()) && meta.interpretation === "local") || meta.relevance === "local") &&
          <p className="text-sm text-muted">{t.recommendation.degraded}</p>}
