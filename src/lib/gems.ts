...
import { BADGE_GEM_REWARDS, GEM_REWARD_KEYS, HIGHER_LOWER_MILESTONES } from "@/constants/gems";
...
interface GemProgress { gamesWon: number; maxStreak: number; higherLowerBest: number }
...
export async function getGemWallet(userId: string): Promise<GemsWallet> {
  return prisma.$transaction(async (tx) => {
    ...
    const [stats, record, received] = await Promise.all([
    ...
    const pending = eligibleGemRewards({ gamesWon: stats?.gamesWon ?? 0, maxStreak: stats?.maxStreak ?? 0, higherLowerBest: record?.bestScore ?? 0 }).filter(({ rewardKey }) => !earnedKeys.has(rewardKey));
