...
Filter-only requests skip embeddings and do not display a description-matching failure. Ratings and feedback influence ordering locally, independently of AI availability; missing genre/director metadata does not count as a usable taste signal. Description/reference requests still expose degradation when interpretation, retrieval or relevance review is unavailable. Retrieval reports `semantic` when a query vector and at least one indexed candidate allow semantic scoring; mixed pools retain unindexed movies without incorrectly reporting a provider failure. `filters` means no usable semantic scoring occurred. This status is not a claim of complete catalog coverage: use the health check for index coverage.

Runtime fallbacks emit a throttled `recommendation_fallback` server log with the stage (`interpretation`, `embedding`, `relevance`) and a safe reason (`missing_configuration`, `authentication`, `credits_exhausted`, `rate_limited`, `timeout`, `invalid_response`, `unavailable`). HTTP status is included when known; prompts, response bodies, credentials and user history are omitted. Use these logs to distinguish provider errors from the historical catalog coverage issue below. Catalog counts recorded below are a dated rollout snapshot, not a current health check.
