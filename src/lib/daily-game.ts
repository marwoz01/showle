...
export function parseDailyAction(body: unknown): DailyAction | null {
  ...
  return input.type === "guess" &&
    Number.isSafeInteger(input.movieId) &&
    Number(input.movieId) > 0
    ? { type: "guess", movieId: Number(input.movieId) }
    : null;
}

export async function applyDailyAction(
  actorId: string,
  dateKey: string,
  action: DailyAction,
  rewarded: boolean,
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (action.type === "guess" && guessIds.includes(action.movieId))
        return existing!;
      if (action.type === "guess" && guessIds.length < MAX_ATTEMPTS)
        guessIds.push(action.movieId);
      const won = guessIds.at(-1) === answer.id;
      const status = won
        ? "won"
        : action.type === "give-up" || guessIds.length >= MAX_ATTEMPTS
          ? "lost"
          : "playing";
      const hintsUsed = [2, 4, 6].filter(
        (threshold) => guessIds.length >= threshold,
      ).length;
      const data = {
        status,
        guessIds,
      ...
      const statData = {
        gamesPlayed,
        gamesWon: (stats?.gamesWon ?? 0) + Number(won),
        currentStreak,
        maxStreak: Math.max(stats?.maxStreak ?? 0, currentStreak),
        lastPlayedDate: dateKey,
        averageGuesses:
          ((stats?.averageGuesses ?? 0) * (stats?.gamesPlayed ?? 0) +
            guessIds.length) /
          gamesPlayed,
      };
      await tx.userStats.upsert({
        where: { userId: actorId },
        update: statData,
        create: { userId: actorId, ...statData },
      });
...
export async function getDailyGameView(
  actorId: string,
  dateKey: string,
  locale: "pl" | "en",
  ...
  const exactLabels = new Set(
    guesses.flatMap((g) =>
      g.comparison.filter((f) => f.status === "exact").map((f) => f.label),
    ),
  );
  return {
    dateKey,
    ...
    revealedPeople: {
      ...(exactLabels.has(t.comparison.director)
        ? { directorProfilePath: answer.directorProfilePath }
        : {}),
      ...(exactLabels.has(t.comparison.leadActor)
        ? {
            cast: answer.cast?.filter(
              (member) => member.name === answer.leadActor,
            ),
          }
        : {}),
    },
