...
export async function interpretRecommendation(text: string): Promise<RecommendationIntent> {
  ...
  try {
    ...
    if (!isRecord(raw) || typeof raw.queryEnglish !== "string" || !raw.queryEnglish.trim() || raw.queryEnglish.length > 1000) {
      throw new SyntaxError("invalid_interpretation");
    }
    ...
    reportRecommendationFallback("interpretation", error);
    return local;
  }
}
