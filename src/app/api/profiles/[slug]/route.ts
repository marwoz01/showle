...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const profile = await prisma.userProfile.findUnique({
      ...
      select: { userId: true, isPublic: true, publicSlug: true, displayName: true, bio: true, avatarUrl: true, favoriteMovies: true },
    });
    if (!profile || !await canReadSocialProfile(profile, userId)) return NextResponse.json({ error: "not_found" }, { status: 404, headers });
    ...
    if (!current || current.userId !== profile.userId || !await canReadSocialProfile(current, userId))
      return NextResponse.json({ error: "not_found" }, { status: 404, headers });
