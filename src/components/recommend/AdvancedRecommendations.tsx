...
export default function AdvancedRecommendations() {
  ...
  const [preferences, setPreferences] = useState<RecommendationPreference>({
    source: searchParams.get("source") === "watchlist" ? "watchlist" : "catalog",
  ...
  useEffect(() => {
    ...
    busy.current = false;
    setRemaining(null); setQuotaLimit(null); setWatchlistCount(null); setResults([]); setMeta(null); setExcludeIds([]); setView("form");
    return () => { requestRef.current?.abort(); };
  }, [taste.userId, taste.isLoaded]);

  useEffect(() => {
    if (!taste.isLoaded || view !== "form") return;
    ...
    const refresh = () => {
      ...
      fetch("/api/recommend", { cache: "no-store", signal: ac.signal })
        ...
        .then((data) => { if (data && !ac.signal.aborted) { setRemaining(data.remaining); setQuotaLimit(data.limit); setWatchlistCount(data.watchlistCount ?? null); } })
        .catch(() => {});
    };
    refresh();
    window.addEventListener("focus", refresh);
    return () => { current?.abort(); window.removeEventListener("focus", refresh); };
  }, [taste.userId, taste.isLoaded, view]);

  async function handleSubmit(prefs: RecommendationPreference, selectedReference = reference, excluded = excludeIds) {
    if (busy.current || taste.pending.length) return;
    busy.current = true;
    const ac = new AbortController();
    requestRef.current = ac;
    const timer = setTimeout(() => ac.abort("timeout"), 25000);
    setPreferences(prefs); setReference(selectedReference); setExcludeIds(excluded); setMeta(null); setView("loading"); setErrorType(""); setFeedbackMessage("");
    try {
      const response = await fetch("/api/recommend", { method: "POST", signal: ac.signal,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...prefs, locale, exclude: excluded, positiveIds: taste.positiveIds, negativeIds: taste.negativeIds }),
      });
      ...
      setExcludeIds([...new Set([...excluded, ...data.recommendations.map((r: Recommendation) => r.movie.id)])].slice(-MAX_RECOMMEND_EXCLUDES));
      setView("results");
  ...
  const errors: Record<string, string> = {
    watchlist_login_required: t.recommendation.watchlistLogin, watchlist_empty: t.recommendation.watchlistEmpty,
    watchlist_no_results: t.recommendation.watchlistNoResults,
  ...
  return (
    <div ref={topRef} className="relative mx-auto max-w-6xl space-y-7">
      ...
      {view === "form" && <PreferenceForm initial={preferences} initialReference={reference} onSubmit={(prefs, selected) => void handleSubmit(prefs, selected, [])} remaining={remaining} quotaLimit={quotaLimit} signedIn={Boolean(taste.userId)} watchlistCount={watchlistCount} />}
      ...
      {view === "error" && <div role="alert" className="soft-card space-y-5 rounded-2xl p-8 text-center">
        ...
        {!!meta?.watchlistUnavailable && <p className="text-sm text-muted">{t.recommendation.watchlistIncomplete(meta.watchlistUnavailable)}</p>}
        {meta && meta.matching === "filters" && <p className="text-sm text-muted">{t.recommendation.degraded}</p>}
        <div className="flex flex-wrap justify-center gap-3">
          {["daily_limit_anon", "watchlist_login_required"].includes(errorType) && <Link href="/sign-in" className={actionClass}>{t.nav.login}</Link>}
          {["watchlist_empty", "watchlist_no_results"].includes(errorType) && <Link href="/collection?tab=watchlist" className={actionClass}>{t.collection.title}</Link>}
          {!["daily_limit_reached", "daily_limit_anon", "pool_exhausted", "no_results", "watchlist_empty", "watchlist_no_results", "watchlist_login_required"].includes(errorType) &&
