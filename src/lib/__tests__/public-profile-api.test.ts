...
const mocks = vi.hoisted(() => ({ userId: "viewer" as string | null, allowed: true, profile: vi.fn(), summary: vi.fn(), badges: vi.fn(), collection: vi.fn(), feedback: vi.fn(), catalog: vi.fn() }));
...
const profile = { userId: "friend", publicSlug: "public-friend", displayName: "Friend", bio: "Hello", avatarUrl: null, favoriteMovies: [], genres: [], excludedGenres: [], providerIds: [], maxRuntime: null };
...
describe("public profile endpoints", () => {
  it("only looks up explicitly public profiles and returns a no-store whitelist", async () => {
    const response = await publicProfile(request(), params);
    expect(response.status).toBe(200);
    expect(response.headers.get("Cache-Control")).toContain("no-store");
    expect(mocks.profile.mock.calls[0][0].where).toEqual({ publicSlug: "public-friend", isPublic: true });
