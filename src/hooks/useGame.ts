...
export function useGame() {
    ...
    try {
      ...
      if (!userId && !view.guesses.length && view.status === "playing") {
        ...
        if (saved?.dateKey === view.dateKey && Array.isArray(saved.guessIds)) {
          ...
          try {
            localStorage.removeItem("showle-daily-movie");
          } catch {
            /* optional storage */
          }
        }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (game && game.dateKey !== getTodayKey() && !busy.current) {
        setCelebrate(false);
        void refresh();
      }
    }, 15000);
  ...
  const act = useCallback(
    async (action: { type: string; movieId?: number }) => {
      ...
      try {
        const response = await fetch(
          `/api/game/state?lang=${locale}&dateKey=${game.dateKey}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(action),
